...
def get_details(image_file, class_name, roi_id):
    ...
    res = ollama.chat(
	    ...
	    messages=[
		    {
			    'role': 'user',
			    'content': f"""Describe a {class_name} recognized inside the rectangle.
                Make it very short and concise.
                Very important: Do not mention any rectangle nor image - it's only technical.
                Instead use words like 'I can see...'
                """,
			    'images': [image_file],
		    }
	    ]
    ...
    while True:
        ...
        for box in results[0].boxes:
            ...
            if id not in recognized_objects:
                recognized_objects[id] = "./images/roi" + str(int(id)) + ".jpg"
